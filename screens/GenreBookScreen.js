...
export default function GenreBookScreen({navigation,route}) {    
  const [book,setBook]    = useState({data : [{'name' : 'Загрузка'}]});
  const [pages,setPage]   = useState();
  const [SB,setSB]        = useState();

  const getAutoBook = (g_code) => {
    setBook(
      {data : {'name' : 'Загрузка'}}
    )
    setSB({})
    fetch('http://flibapi.tmweb.ru/get_book_genre/'+g_code, { method: 'GET',})
  ...
  useEffect(() => {
    getAutoBook(route.params.g_code,0)
  }, [])
  return (
